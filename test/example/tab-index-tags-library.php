...
<?php 
    $url = Core::getInstance()->api.'/book/data/index/tags/library/'.$datalogin['username'].'/'.$datalogin['token'].'/'.$page.'/'.$itemsperpage.'/?query='.rawurlencode($search);
    ...
    if (!empty($data))
        {
            if ($data->{'status'} == "success")
            {
                ...
                foreach ($data->results as $name => $value) 
	            {
					...
					echo '<td><a href="modul-book-library.php?m=13&page=1&itemsperpage=12&search='.$value->{'Tags'}.'">' . $value->{'Tags'} .'</a></td>';
        			echo '</tr>';              
                }
                echo '</tbody>
                </table>';
