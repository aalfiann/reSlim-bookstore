                ...
                <div class="row">
                    <?php 
                        if (isset($_POST['submitnewlanguage']))
                        {
                            ...
                            Core::createProcess(Core::getInstance()->api.'/book/language/new',$post_array,'New Language');
                        }
                    ?>
                    <!-- Start Modal -->
                          ...
                          <div class="modal-dialog" role="document">
                            <div class="modal-content">
                              ...
                              <div class="modal-footer">
                                ...
                                <button type="submit" name="submitnewlanguage" class="btn btn-primary">Submit</button>
                              </div>
                              </form>
                            </div>
                          </div>
                    ...
                    <div class="col-md-12">
                        <div class="card card-plain">
                            <div class="header">
                                <button type="submit" class="btn btn-wd" data-toggle="modal" data-target="#myModal">Add new Language</button>
                            </div>
                        </div>
                    </div>
<?php 
    $url = Core::getInstance()->api.'/book/language/data/search/'.$datalogin['token'].'/'.$_GET['page'].'/'.$_GET['itemsperpage'].'/?query='.$_GET['search'];
            ...
            if ($data->{'status'} == "success")
            {
                foreach ($data->results as $row => $value) {
                    if (isset($_POST['submitupdatelanguage'.$value->{'LanguageID'}]))
                    {
                        ...
                        Core::updateProcess(Core::getInstance()->api.'/book/language/update',$post_array,'Language');
                ...
                foreach ($data->results as $row => $value) {
                    if (isset($_POST['submitdeletelanguage'.$value->{'LanguageID'}]))
                    {
                        ...
                        Core::deleteProcess(Core::getInstance()->api.'/book/language/delete',$post_array,'from Language');
                        echo Core::reloadPage();
                    }
                }
                ...
                echo '<div class="col-md-12">
                        <div class="card card-plain">
                            <div class="header">
                                <h4 class="title text-uppercase">Data Language</h4>
                ...
                foreach ($data->results as $name => $value) 
	            {
        			...
        			echo '<td><a href="#" data-toggle="modal" data-target="#'.$value->{'LanguageID'}.'"><i class="ti-pencil"></i> Edit</a></td>';
	    	    	echo '</tr>';              
                }
                echo '</tbody>
                </table>';
                ...
                echo $pagination->makePagination($data,$_SERVER['PHP_SELF'].'?m=9&search='.$_GET['search']);
                        ...
                        <div class="modal fade" id="'.$value->{'LanguageID'}.'" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                          <div class="modal-dialog" role="document">
                            <div class="modal-content">
                              ...
                              <form method="post" action="'.$_SERVER['PHP_SELF'].'?m=9&page='.$_GET['page'].'&itemsperpage='.$_GET['itemsperpage'].'&search='.$_GET['search'].'">
                              <div class="modal-body">
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            <label>Language ID</label>
                                            <input name="languageid" type="text" placeholder="Input your language id here..." class="form-control border-input" value="'.$value->{'LanguageID'}.'" readonly>
                                        </div>
                                    </div>
                                    <div class="col-lg-12">
                                        <div class="form-group">
                                            <label>Language Name</label>
                                            <input name="name" type="text" placeholder="Input your language name here..." class="form-control border-input" value="'.$value->{'Name'}.'" required>
                                        </div>
                                    </div>
                                </div>
                              </div>
                              <div class="modal-footer">
                                <button type="submit" name="submitdeletelanguage'.$value->{'LanguageID'}.'" class="btn btn-danger pull-left">Delete</button>
                                <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
                                <button type="submit" name="submitupdatelanguage'.$value->{'LanguageID'}.'" class="btn btn-primary">Update</button>
                              </div>
                              </form>
                            </div>
                          </div>
