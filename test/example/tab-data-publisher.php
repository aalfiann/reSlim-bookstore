                ...
                <div class="row">
                    <?php 
                        if (isset($_POST['submitnewpublisher']))
                        {
                            ...
                            Core::createProcess(Core::getInstance()->api.'/book/publisher/new',$post_array,'New Publisher');
                        }
                    ?>
                    <!-- Start Modal -->
                          ...
                          <div class="modal-dialog" role="document">
                            <div class="modal-content">
                              ...
                              <div class="modal-footer">
                                ...
                                <button type="submit" name="submitnewpublisher" class="btn btn-primary">Submit</button>
                              </div>
                              </form>
                            </div>
                          </div>
                    ...
                    <div class="col-md-12">
                        <div class="card card-plain">
                            <div class="header">
                                <button type="submit" class="btn btn-wd" data-toggle="modal" data-target="#myModal">Add new Publisher</button>
                            </div>
                        </div>
                    </div>
<?php 
            ...
            if ($data->{'status'} == "success")
            {
                foreach ($data->results as $row => $value) {
                    if (isset($_POST['submitupdatepublisher'.$value->{'PublisherID'}]))
                    {
                        ...
                        Core::updateProcess(Core::getInstance()->api.'/book/publisher/update',$post_array,'Publisher');
                ...
                foreach ($data->results as $row => $value) {
                    if (isset($_POST['submitdeletepublisher'.$value->{'PublisherID'}]))
                    {
                        ...
                        Core::deleteProcess(Core::getInstance()->api.'/book/publisher/delete',$post_array,'from Publisher');
                        echo Core::reloadPage();
                    }
                }
                ...
                echo '<div class="col-md-12">
                        <div class="card card-plain">
                            <div class="header">
                                <h4 class="title text-uppercase">Data Publisher</h4>
                ...
                foreach ($data->results as $name => $value) 
	            {
        			...
        			echo '<td><a href="#" data-toggle="modal" data-target="#'.$value->{'PublisherID'}.'"><i class="ti-pencil"></i> Edit</a></td>';
	    	    	echo '</tr>';              
                }
                echo '</tbody>
                </table>';
                        ...
                        <div class="modal fade" id="'.$value->{'PublisherID'}.'" tabindex="-1" role="dialog" aria-labelledby="myModalLabel">
                          <div class="modal-dialog" role="document">
                            <div class="modal-content">
                              ...
                              <div class="modal-footer">
                                <button type="submit" name="submitdeletepublisher'.$value->{'PublisherID'}.'" class="btn btn-danger pull-left">Delete</button>
                                <button type="button" class="btn btn-default" data-dismiss="modal">Cancel</button>
                                <button type="submit" name="submitupdatepublisher'.$value->{'PublisherID'}.'" class="btn btn-primary">Update</button>
                              </div>
                              </form>
                            </div>
                          </div>
